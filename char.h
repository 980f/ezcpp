...
public:
  ...
  /** @returns lower case of this */
  char asLower() const noexcept;
  /** @returns upper case of this */
  char asUpper() const noexcept;
  /** converts to lower case and @returns whether that caused a change */
  bool toLower() noexcept;
  /** converts to upper case and @returns whether that caused a change */
  bool toUpper() noexcept;

  /** @returns this conditionally toUpper'd (which==1) or toLower'd (which==-1) or unmodified (which==0) */
