...
class IntegerTwiddler {
  ...
  public:
    ...
    /** @returns either @param whole or ++whole. Works on any class for whch operator ++(int) is defined  */
    template<typename Intish> Intish operator()(Intish whole) {
      return pwm() ? ++whole : whole;
    }
