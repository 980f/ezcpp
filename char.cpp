...
*/
static const char *SeaScapes = "a\a"
                               "b\b"
                               "c\x03"
                               "f\f"
                               "n\n"
...
/** @returns the c-escape sibling of this char.
...
/** converts this to lower case and @returns whether that caused a change */
...
unsigned Char::hexDigit() const noexcept {
  unsigned trusting = asLower() - '0'; // tolower then subtract char for zero.
  if (trusting > 9) {
    trusting -= 39; // ~ 'a' -'0' + 10
  }
  return trusting;
